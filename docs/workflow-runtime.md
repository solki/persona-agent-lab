...
The Runs page at `/runs` lists active run history by default, links to monitor, trace, executions, and token usage pages, and provides confirmed archive for old run results. Archiving a run hides it from the default Active runs list, while activation restores archived runs to Active runs. Both preserve trace, execution, token usage, feedback/evaluation, proposed-memory, and learning-event records tied to that run. They do not delete agents, workflows, souls, tools, contexts, active approved agent memories, or proposed-memory records. Use the Runs page archive filter to inspect or activate archived runs. Permanent delete is limited to archived runs that pass backend safety checks.
